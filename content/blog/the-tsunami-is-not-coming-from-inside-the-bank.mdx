...
date: "2026-02-19"
theme: "demand-side"
excerpt: "When every mortgage seeker sends an AI agent, inbound volume multiplies tenfold and revenue does not. The shift that hits first comes from outside the bank."
standfirst: "The agentic shift most banks prepare for is internal. The one that hits first comes from clients: when every mortgage seeker sends an agent, inbound volume multiplies tenfold and revenue does not. The economics force a redesign."
---
...
Before the agent, look at the funnel that already exists. The waste in the current model is larger than most banks acknowledge.
...
Each of those 50'000 transactions generates multiple mortgage applications. A buyer typically contacts three to five banks, and many people who request quotes never buy at all: they are searching, comparing, testing affordability, or losing out in competitive bidding. On comparison platforms, quote-to-close ratios of ten to one or higher are common (author's estimate; no platform publishes the figure).
...
**Today:** Based on industry estimates, processing one mortgage application costs roughly CHF 1'500. Assume, for illustration, that a won mortgage is worth CHF 20'000 to the bank over its life. With a conversion rate of roughly one in four (accounting for the existing funnel leakage), the expected value per application is around CHF 5'000. The unit economics are manageable.

**In the agentic world:** The conversion rate drops to one in forty or worse. The cost per application has not changed. Expected value per application: CHF 500, against a cost of CHF 1'500. **On average, every application now loses the bank CHF 1'000.**
