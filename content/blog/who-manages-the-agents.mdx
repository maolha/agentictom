...
date: "2026-04-02"
theme: "the-model"
excerpt: "Who is responsible for a workforce that is not human? Neither IT nor HR can own it alone. It is a design decision most banks have not yet made."
standfirst: "A workforce that is not human still needs management: onboarding, performance review, decommissioning. Neither IT nor HR can own it alone. Within the three lines of defence, this is an organisational design decision most banks have not yet made."
---
...
[FINMA Guidance 08/2024](/finma-guidance-08-2024) creates a hard constraint: **responsibility for decisions cannot be delegated to AI.** A named human must be accountable for every decision an agent makes.
...
Someone must define the agent's scope, review its performance, decide when to expand or restrict its autonomy, and decommission it when the regulatory environment changes. Someone must be accountable when something goes wrong, and accountable in the conversation with FINMA rather than in theory.

## The models that don't work

**"IT manages it."** This is the default at most banks today. IT deployed the system, so IT manages the system. The problem is straightforward. An agent that autonomously prices mortgages is a decision-maker operating within a policy defined by the business, and IT manages infrastructure rather than decision-making authority. IT can manage the platform. **It cannot own the policy envelope.**
...
The answer is a full **governance model** rather than a single function.
...
One part remains unsolved.

CyberArk's 2025 Identity Security Landscape puts machine identities at 82 for every human across the organisations it surveyed. Even if only a fraction of those are autonomous agents today, the trajectory is clear. **The ratio of agents to humans will grow faster than any organisation can scale its oversight.**
...
The answer is the same concept already on [this site](/blog/escalation-by-design): **performance envelopes.** You define the expected operating parameters (error rates, escalation frequency, outcome distributions, decision confidence) and monitor against them, instead of reviewing every decision. The agent operates within the envelope. When it drifts outside, it triggers review.

This resembles how a bank manages a trading desk. You do not approve every trade. You set limits, monitor positions, and intervene on exceptions. **The governance model for agents is closer to market risk management than to traditional HR.** The HR metaphor opens the conversation. The risk management framework closes it.
...
**Agent governance needs a concrete reporting line.** Someone must chair the meeting, set the standards, and hold the authority to shut an agent down.
...
The metaphor of "HR for agents" is useful for starting the conversation. **Finishing it takes an operating model**: one that fits within the three lines of defence, satisfies FINMA's accountability requirements, and scales beyond the first ten agents.
